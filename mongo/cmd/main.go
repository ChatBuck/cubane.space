...
func initMongoDB() {
	...
	blockCollection = client.Database("blockchainDB").Collection("blocks")
	transactionCollection = client.Database("blockchainDB").Collection("transactions")
	fmt.Println("Connected to MongoDB!")
}

func saveBlockToMongoDB(block Block) {
	_, err := blockCollection.InsertOne(context.TODO(), block)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("Block added to MongoDB:", block)

	for _, txn := range block.Transactions {
		saveTransactionToMongoDB(txn)
	}
}

func saveTransactionToMongoDB(txn Transaction) {
	_, err := transactionCollection.InsertOne(context.TODO(), txn)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("Transaction added to MongoDB:", txn)
}
	...
	for {
		...
		if str != "\n" {
			...
			if len(chain) > len(Blockchain) {
				...
				// Save new blocks to MongoDB
				for i := oldLen; i < len(chain); i++ {
					saveBlockToMongoDB(chain[i])
				}
			}
			mutex.Unlock()
		}
...
func handleTransaction(w http.ResponseWriter, r *http.Request) {
	...
	if isBlockValid(newBlock, Blockchain[len(Blockchain)-1]) {
		...
		saveBlockToMongoDB(newBlock) // Save new block to MongoDB
		mutex.Unlock()
	}

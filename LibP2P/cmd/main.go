...
func writeData(rw *bufio.ReadWriter) {
	...
	for {
		...
		transactionData := []Transaction{{Data: []byte(sendData)}}
		newBlock := generateBlock(Blockchain[len(Blockchain)-1], transactionData)
...
func main() {
	...
	if *target == "" {
		...
		//ttl := time.Duration(0) // Set to 0 for effectively permanent
		ttl := time.Hour // Set to 0 for effectively permanent
		...
		log.Println("Opening stream")
		log.Println("Converted target peerId : ", peer2.ID(peerid))
...
// helper function to convert transactions to a string for hashing
func transactionsToString(transactions []Transaction) string {
	var transactionStrings []string
	for _, txn := range transactions {
		transactionStrings = append(transactionStrings, string(txn.Data))
	}
